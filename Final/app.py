...
import os
from src.angle_calculation_wdisplay import process_image
from werkzeug.utils import secure_filename
import ast
from src.position_classification import frobenius
from src.tomatrix import pose_to_matrix
from pose import Pose
from animation_creation import sachaleboss
...
def upload():
    if request.method == 'POST' and 'input_file' in request.files:
        ...
        return render_template('zeub.html', zeub=s)

    return render_template('index.html')

@app.route('/sacha', methods=['POST'])
def sacha():
    if request.method == 'POST' and 'input_file' in request.files:
        file = request.files['input_file']
        if file.filename == '':
    ...
    with open("static/AnimationFile.txt","w") as f:
        f.writelines(["%s\n" % item for item in sachaleboss(os.path.join(app.config['UPLOAD_FOLDER'],filename))])
    os.remove(f'{UPLOAD_FOLDER}/{file.filename}')
    
    return render_template('sacha.html')
